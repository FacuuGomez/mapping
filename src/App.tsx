...
import { fetchAddresses, fetchCustomerLocations } from "./utils/fetchAddresses";
...
const App = () => {
	...
	const [selectedMarker, setSelectedMarker] = useState<CustomerInfo | null>(
		null
	);
	const [coordinates2, setCoordinates2] = useState<CustomerInfo[]>([]);
	...
	useEffect(() => {
		...
		if (coordinates.length === 0) {
			...
			fetchCustomerLocations("/data.csv")
				.then((customerLocations: CustomerLocation[]) => {
					console.log("customerLocations", customerLocations);
					setCustomerLocations(customerLocations);
					setCenter({
						lat: customerLocations[0].latLng?.lat || 0,
						lng: customerLocations[0].latLng?.lng || 0,
					});
				})
				.catch((error: Error) =>
					console.error("Error fetching or geocoding addresses:", error)
	...
	return (
		<div className='fixed top-0 flex w-full h-screen'>
			...
			>
				...
				{showCustomers &&
					customerLocations &&
					customerLocations.map((coordinate, index) => (
						<Marker
							key={index}
							position={coordinate.latLng || center}
							icon={{
							...
							onClick={() =>
								setSelectedMarker({
									lat: coordinate.latLng?.lat || center.lat,
									lng: coordinate.latLng?.lng || center.lng,
									name: `${coordinate.firstName} ${coordinate.lastName}`,
									address: `${coordinate.address1} ${coordinate.city}, ${coordinate.state}`,
				...
				{selectedMarker && (
					...
					>
						<div className='text-black'>
							<h3 className='font-bold'>{selectedMarker.name || "Cliente"}</h3>
							<p>{selectedMarker.address || "Dirección no disponible"}</p>
							<div className='mt-2'>
								<p>
									<strong>Phone:</strong>{" "}
									{selectedMarker.phone || "No disponible"}
								</p>
								<p>
									<strong>Route:</strong>{" "}
									{selectedMarker.route || "No asignada"}
								</p>
								<p>
									<strong>State:</strong>{" "}
									{selectedMarker.state || "No disponible"}
								</p>
								<p>
									<strong>Zip:</strong> {selectedMarker.zip || "No disponible"}
								</p>
							</div>
						</div>
					</InfoWindow>
